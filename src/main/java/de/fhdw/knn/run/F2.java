...
public class F2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Network network = new Network(42, 1, 100, 100, 100, 1);

        LossFunction lossFunction = LossFunction.MEAN_SQUARED_ERROR;
        StopFunction stopFunction = new EarlyStopping(0.0001, 8);
        OptimizationFunction optimizationFunction = new GradientDescent(network, lossFunction, 0.2);

        Trainer trainer = new Trainer(network, 50, true, lossFunction, stopFunction, optimizationFunction);
        trainer.train(data);
